...
namespace sakeny.Controllers
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        public PostsController(IpostRepo postRepo , IMapper mapper , IFaviourateRepo favouriteRepo , IUserInfoRepository userInfoRepository)
        {
            _postRepo = postRepo;
            _mapper = mapper;
		...
		{
			// Extract the user ID from the claims. Adjust this as necessary based on how you store the user ID.
			...
			var posts = await _postRepo.GetAllPostsAsync(userId);
		...
		public async Task<ActionResult<IEnumerable<PostDto>>> SearchPosts(
	          ...
	          [FromQuery] int? priceAi,
	          [FromQuery] bool? statue)
		{
			PostDto searchCriteria = new PostDto
			{
